...
interface BaseCalendarProps {
  ...
  monthsToShow?: 1 | 2 | 3;
  /** Chỉ chọn tháng + năm (không lưới ngày); chọn tháng là xong */
  monthYearOnly?: boolean;
  /** iconOnly = nút vuông chỉ biểu tượng lịch (kết hợp ô gõ bên cạnh) */
  calendarTriggerVariant?: "default" | "iconOnly";
      ...
      >
        ...
        <div className="flex flex-col items-center gap-1">
          <button
            ...
            disabled={disabled}
            className="pointer-events-auto rounded-lg p-1.5 transition-colors hover:bg-[#a1001f]/15 hover:text-[#a1001f] disabled:opacity-50"
          ...
          <input
            ...
            disabled={disabled}
            className={cn(
              "pointer-events-auto w-12 rounded border-none bg-transparent text-center font-bold font-mono focus:outline-none focus:ring-2 focus:ring-[#a1001f]",
              sizes.header,
            )}
            aria-label="Hours"
            maxLength={2}
        ...
        <span className={cn("font-bold text-gray-500", sizes.header)}>
          :
        </span>
        ...
        {!use24Hour && (
          <button
            ...
            disabled={disabled}
            className="pointer-events-auto ml-2 rounded-lg bg-gray-200 px-3 py-2 font-semibold text-sm transition-colors hover:bg-gray-300/80 disabled:opacity-50"
        ...
        {localeStrings.monthsShort.map((month, index) => {
          ...
          return (
            <motion.button
              ...
              disabled={isDisabled}
              className={cn(
                "rounded-lg px-2 py-3 font-medium text-sm transition-all focus:outline-none focus:ring-2 focus:ring-[#a1001f]",
                isSelected
                  ? "bg-[#a1001f] text-white shadow-md hover:bg-[#8a0019] hover:text-white"
                  : "text-gray-900 hover:bg-[#a1001f]/15 hover:text-[#a1001f]",
      ...
      <div className="space-y-2 p-2" role="listbox" aria-label="Select year">
        <div className="mb-2 flex items-center justify-between">
          <button
            ...
            className="rounded-lg p-1.5 transition-colors hover:bg-[#a1001f]/15 hover:text-[#a1001f]"
...
const PresetsPanel = React.memo(
  ({
    ...
    <div
      className="mr-3 min-w-[140px] space-y-1 border-gray-200 border-r pr-3"
      role="group"
      aria-label="Quick date presets"
    >
      <span className="mb-2 block font-semibold text-gray-500 text-xs uppercase tracking-wider">
        Quick Select
      </span>
      {presets.map((preset, index) => (
        <motion.button
          ...
          disabled={disabled}
          className="w-full rounded-lg px-3 py-2 text-left text-sm transition-colors hover:bg-[#a1001f]/15 hover:text-[#a1001f] focus:outline-none focus:ring-2 focus:ring-[#a1001f] disabled:opacity-50"
        >
          {preset.label}
        </motion.button>
      ))}
...
const CalendarContent = React.memo(
  ({
    ...
    const handleMonthSelect = useCallback(
      (month: number) => {
        const newDate = monthYearOnly
          ? new Date(getYear(currentMonth), month, 1)
        ...
        if (monthYearOnly) {
          if (mode === "single") {
            onChange?.(newDate);
            announce(
              `Selected ${format(newDate, "MMMM yyyy", { locale })}`,
            );
            if (closeOnSelect) onClose?.();
          }
          return;
    ...
    const goToToday = useCallback(() => {
      ...
      if (monthYearOnly) {
        ...
        setDirection(isAfter(first, startOfMonth(currentMonth)) ? 1 : -1);
        setCurrentMonth(first);
        if (mode === "single") {
          onChange?.(first);
          announce(`Selected ${format(first, "MMMM yyyy", { locale })}`);
          if (closeOnSelect) onClose?.();
        }
        return;
      }
          ...
          <motion.button
            ...
            className={cn(
              ...
              !isSelected &&
                ...
                !inRange &&
                "text-gray-900 hover:bg-[#a1001f]/15 hover:text-[#a1001f]",
              isSelected && "bg-[#a1001f] text-white shadow-sm hover:bg-[#8a0019] hover:text-white",
              isTodayDate &&
                !isSelected &&
                "ring-2 ring-[#a1001f] ring-offset-2 ring-offset-white hover:bg-[#a1001f]/15 hover:text-[#a1001f]",
              inRange && "rounded-none bg-[#a1001f]/15 hover:bg-[#a1001f]/25",
              isFocused && "ring-2 ring-gray-400 ring-offset-1",
            )}
          >
            ...
            {isTodayDate && !isSelected && (
              <span className="absolute bottom-0.5 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full bg-[#a1001f]" />
            )}
            ...
            {highlight && (
              <span
                ...
                style={{
                  backgroundColor: highlight.color || "#a1001f",
                }}
          ...
          >
            {isSecondary && (
              <div className="mb-2 flex h-8 items-center justify-center">
                <span
                  className={cn("font-semibold text-gray-900", sizes.header)}
                >
            ...
            >
              {showWeekNumbers && (
                <div
                  className={cn(
                    sizes.cell,
                    "flex items-center justify-center font-medium text-gray-500 text-xs",
                  )}
                  role="columnheader"
                  tabIndex={-1}
                >
              ...
              {getWeekDays().map((day, i) => (
                <div
                  ...
                  className={cn(
                    sizes.cell,
                    "flex items-center justify-center font-semibold text-gray-500 text-xs",
                  )}
                  tabIndex={-1}
                >
                  {day}
    ...
    return (
      <motion.div
        ...
        tabIndex={0}
        role="application"
        aria-label={monthYearOnly ? "Month and year" : "Calendar"}
        initial={
        ...
        transition={{ duration: prefersReducedMotion ? 0 : 0.2 }}
        className={cn(
          "pointer-events-auto overflow-hidden rounded-xl border border-gray-200 bg-white text-gray-900 shadow-black/10 shadow-xl focus:outline-none focus:ring-2 focus:ring-[#a1001f]",
          sizes.container,
          calendarWidth,
        ...
        <div className="flex-1">
          ...
          <div className="mb-3 flex items-center justify-between">
            <div className="flex items-center gap-0.5">
              <button
                ...
                disabled={disabled}
                className="rounded-lg p-1.5 transition-colors hover:bg-[#a1001f]/15 hover:text-[#a1001f] disabled:opacity-50"
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                disabled={disabled}
                className={cn(
                  "rounded-lg px-2 py-1 font-bold transition-colors hover:bg-[#a1001f]/15 hover:text-[#a1001f]",
                  sizes.header,
                )}
              ...
              <button
                ...
                onClick={() =>
                  monthYearOnly
                    ? handleViewChange(view === "years" ? "months" : "years")
          ...
          {showTime && mode === "single" && view === "days" && (
            <button
              ...
              disabled={disabled}
              className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg bg-[#a1001f]/5 py-2 font-medium text-sm text-[#a1001f] transition-colors hover:bg-[#a1001f]/15 hover:text-[#8a0019] disabled:opacity-50"
            >
          ...
          {(showTodayButton ||
            ...
            (monthYearOnly
              ...
              : view === "days") && (
              <div className="mt-4 flex items-center justify-between border-gray-200 border-t pt-3">
                <div className="flex items-center gap-2">
                  {showTodayButton && (
                    <button
                      ...
                      onClick={goToToday}
                      disabled={disabled}
                      className="flex items-center gap-1 rounded-md px-3 py-1.5 font-semibold text-xs transition-colors hover:bg-[#a1001f]/15 hover:text-[#a1001f] disabled:opacity-50"
                    >
                ...
                {showClearButton && value && (
                  <button
                    ...
                    onClick={handleClear}
                    disabled={disabled}
                    className="flex items-center gap-1 rounded-md px-3 py-1.5 font-medium text-gray-500 text-xs transition-colors hover:bg-[#a1001f]/15 hover:text-[#a1001f] disabled:opacity-50"
                  >
                    <X className="h-3 w-3" />
                    {localeStrings.clear}
                  </button>
...
export function AnimatedCalendar({
  ...
  calendarTriggerVariant = "default",
  ...props
}: AnimatedCalendarProps) {
  const [isOpen, setIsOpen] = useState(false);
  /** Remount lịch mỗi lần mở — monthYearOnly luôn bắt đầu từ lưới tháng */
  ...
  const sizeClasses =
    calendarTriggerVariant === "iconOnly"
      ? { sm: "h-9 w-9 text-xs", md: "h-10 w-10 text-sm", lg: "h-12 w-12 text-base" }
      : {
  ...
  return (
    <div className="relative">
      <Popover open={isOpen} onOpenChange={handleOpenChange}>
        ...
        <PopoverContent
          className="w-auto border-0 bg-white p-0 shadow-none"
          align="start"
        >
          <CalendarContent
            key={popoverEpoch}
      ...
      {error && errorMessage && (
        <p
          id={errorId}
          className="mt-1.5 flex items-center gap-1 text-red-600 text-xs"
        >
