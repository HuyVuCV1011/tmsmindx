...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const ho_ten = body.ho_ten || body.full_name;
    const dia_chi_email = body.dia_chi_email || body.email;
    const co_so_lam_viec = body.co_so_lam_viec || body.campus;
    const khu_vuc = body.khu_vuc || body.region;
    const hinh_thuc = body.hinh_thuc || body.registration_type;
    const khoi_giang_day = body.khoi_giang_day || body.block_code;
    const thang_dk = body.thang_dk || body.month;
    const nam_dk = body.nam_dk || body.year;
    const dot = body.dot;
    const id_mon = body.id_mon || body.subject_id;
    const ma_mon = body.ma_mon || body.subject_code;
    const id_su_kien = body.id_su_kien || body.schedule_id;
    ...
    if (!ho_ten) {
      return NextResponse.json({ success: false, error: 'ho_ten là bắt buộc' }, { status: 400 });
    }
    if (!id_mon && !ma_mon) {
    ...
    if (!resolvedSubjectId && ma_mon) {
      const subj = await client.query(
        `SELECT id FROM chuyen_sau_monhoc WHERE ma_mon = $1 LIMIT 1`,
        [ma_mon]
      );
    ...
    // Kiểm tra trùng đăng ký (cùng giáo viên + môn + tháng)
    const dupCond: string[] = ['id_mon = $1', `LOWER(TRIM(ma_giao_vien)) = LOWER(TRIM($2))`, 'diem IS NULL'];
    ...
    const insertResult = await client.query(
      `INSERT INTO chuyen_sau_results (
         ...
         thang_dk, nam_dk, dot,
         id_mon, id_su_kien, id_de_thi, dang_ky_luc
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Chỉ cho xóa nếu chưa có điểm (diem IS NULL = chưa thi)
    const result = await pool.query(
      `DELETE FROM chuyen_sau_results
       WHERE id = $1 AND diem IS NULL
