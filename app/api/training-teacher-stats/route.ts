  ...
  try {
    ...
    let query = `
      SELECT 
        ...
        t."Work email" as work_email,
        t."Main centre" as center,
    ...
    if (center) {
      conditions.push(`t."Main centre" ILIKE $${params.length + 1}`);
      params.push(`%${center}%`);
    }
    ...
    query += ' ORDER BY tts.total_score DESC NULLS LAST, t."Full name" ASC';
