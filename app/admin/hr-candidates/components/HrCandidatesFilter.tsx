      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            onClick={onRefresh}
            disabled={refreshing}
            className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-[#f3b4bd] bg-white px-4 text-sm font-semibold text-[#a1001f] shadow-sm transition-all hover:bg-[#a1001f]/5 active:scale-95 disabled:pointer-events-none disabled:opacity-50"
          >
            <RefreshCw
          ...
          <div className="relative group/columns z-20">
            <button
              type="button"
              className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-gray-300 bg-white px-4 text-sm font-semibold text-gray-700 shadow-sm transition-all hover:bg-gray-50 active:scale-95 outline-none focus-visible:ring-4 focus-visible:ring-blue-100"
            >
              Cột mở rộng
            ...
            <div className="absolute right-0 top-full mt-2 hidden w-72 origin-top-right rounded-2xl border border-gray-200 bg-white p-4 shadow-2xl group-hover/columns:block hover:block focus-within:block animate-in fade-in zoom-in-95 duration-200">
              <div className="mb-3 border-b border-gray-100 pb-3">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Chọn tối đa 4 cột data thô từ sheet để hiển thị trực tiếp lên
                  bảng.
                </p>
              </div>
              <div className="max-h-64 space-y-1 overflow-y-auto pr-1 flex flex-col custom-scrollbar">
                {extraColumnOptions.length === 0 ? (
                  <p className="text-xs text-gray-500 py-4 text-center bg-gray-50 rounded-lg">
                    Không tìm thấy cột ngoài luồng.
                  </p>
                ) : (
                    ...
                    >
                      <div className="flex h-5 items-center mt-0.5">
                        <input
                          ...
                          disabled={
                            !selectedExtraColumns.includes(header) &&
                            selectedExtraColumns.length >= 4
                          }
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-4 rounded-xl bg-blue-50/50 border border-blue-100/50 px-4 py-2.5">
        <div className="flex flex-wrap gap-x-6 gap-y-2 text-[11px] font-medium text-gray-500 uppercase tracking-wide">
          <span className="flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-blue-400"></span> Nguồn
            Sheet:{' '}
            <span className="font-bold text-gray-700 normal-case">
              {source?.sheetId?.slice(0, 8)}...
            </span>
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-blue-400"></span> Tab
            GID:{' '}
            <span className="font-bold text-gray-700 normal-case">
              {source?.gid || 'N/A'}
            </span>
        ...
        {topGenStats.length > 0 && (
          <div className="hidden md:flex gap-1.5">
            <span className="text-[11px] font-bold uppercase text-gray-400 mr-1 flex items-center">
              Top GEN:
            </span>
            {topGenStats.slice(0, 4).map(([gen, count]) => (
              <span
                key={gen}
                className="rounded-md bg-white border border-gray-200 px-2 py-0.5 text-[11px] font-bold text-gray-700 shadow-sm"
              >
