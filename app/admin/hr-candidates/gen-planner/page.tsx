...
function CandidatePopupCell({ row, rowIndex, formatDateTime }: {
  row: HrCandidateRow;
  rowIndex: number;
  ...
  return (
    ...
    >
      <p className="text-sm font-semibold text-gray-900">{row.name || 'Chưa có tên'}</p>
...
export default function HrGenPlannerPage() {
  ...
  const [regionUnassignedCandidates, setRegionUnassignedCandidates] = useState(0);
  ...
  const [sourceGenRegionFilter, setSourceGenRegionFilter] = useState<'all' | '1' | '2' | '3' | '4' | '5'>('all');
  ...
  useEffect(() => {
    ...
    if (regionParam === 'south' || regionParam === 'north' || regionParam === 'all') {
  ...
  const fetchBoardData = useCallback(
    async (forceRefresh = false) => {
      ...
      try {
        ...
        const [candidateRes, genRes] = await Promise.all([
          ...
          fetch(`/api/hr/gens?requestEmail=${encodeURIComponent(user.email)}`, { cache: 'no-store' }),
        ]);
        ...
        setRegionUnassignedCandidates(candidateData.genScope?.regionUnassigned ?? 0);

        const scopedEntries: GenEntry[] = Array.isArray(candidateData.genScope?.entries)
        ...
        if (scopedEntries.length > 0) {
          ...
          const fallbackGens: string[] = candidateData.genScope?.availableGens || candidateData.availableGens || [];
    ...
    [user?.email, page, statusFilter, search, sourceGenFilter, sourceGenRegionFilter, regionFilter]
  );
    ...
    if (
      ...
      !availableGenEntries.some(
        (entry) =>
          ...
          (sourceGenRegionFilter === 'all' || entry.regionCode === sourceGenRegionFilter)
      )
    ) {
  ...
  const submitCreateGen = async (rawGenName: string, isAuto = false) => {
    ...
    try {
      ...
      toast.success(isAuto ? `Đã tự động tạo GEN ${data.gen}.` : `Đã thêm GEN ${data.gen}.`);
      setNewGenName('');
  ...
  const handleStatusCardFilter = (status: 'all' | 'assigned' | 'unassigned') => {
